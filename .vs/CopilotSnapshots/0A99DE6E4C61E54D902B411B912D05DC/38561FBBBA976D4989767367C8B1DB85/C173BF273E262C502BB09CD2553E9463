    ...
    public partial class ClaimVoucher : System.Web.UI.Page
    {
        ...
        private void BindVouchers()
        {
            ...
            using (var con = new SqlConnection(connStr))
            {
                ...
                // Get vouchers and claimed status, but only show vouchers that are not currently used
                using (var cmd = new SqlCommand(@"
                    ...
                    FROM Voucher v
                    LEFT JOIN CustomerVoucher cv ON v.VoucherID = cv.VoucherID AND cv.CustomerID = @CustomerID AND (cv.IsUsed = 0 OR cv.IsUsed IS NULL)
